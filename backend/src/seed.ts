...
const existingDrinksCount = await AppDataSource.getRepository(Drink).count(); // düzgün repository
        ...
        if (existingProductsCount > 0) {
            ...
            const productsData = [
                ...
                { name: "Classic Combo", price: 13.50, category: ProductCategory.COMBOS, description: "Classic Dog + French Fries + Drink", ingredients: [{ name: "Beef Hotdog", qty: 1 }, { name: "Bun", qty: 1 }, { name: "Ketchup", qty: 50 }, { name: "Mustard", qty: 50 }, { name: "Potatoes", qty: 200 }, { name: "Cola", qty: 1 }] },
                { name: "Cheesy Combo", price: 16.00, category: ProductCategory.COMBOS, description: "Cheese Dog + Mozzarella Sticks (4 pcs) + Drink", ingredients: [{ name: "Beef Hotdog", qty: 1 }, { name: "Bun", qty: 1 }, { name: "Melted Cheese", qty: 50 }, { name: "Mozzarella", qty: 200 }, { name: "Cola", qty: 1 }] },
                { name: "Meat Lover’s Double Pack", price: 32.00, category: ProductCategory.COMBOS, description: "BBQ Crunch Dog (Double) + Spicy Dog (Double) + French Fries + Chicken Nuggets (4 pcs) + 2 Drinks", ingredients: [{ name: "Beef Hotdog", qty: 4 }, { name: "Bun", qty: 2 }, { name: "BBQ Sauce", qty: 50 }, { name: "Crispy Onions", qty: 50 }, { name: "Hot Sauce", qty: 50 }, { name: "Jalapeños", qty: 50 }, { name: "Potatoes", qty: 200 }, { name: "Chicken Pieces", qty: 4 }, { name: "Cola", qty: 2 }] },
            ];
            const inventories = await inventoryRepository.find();
            const inventoryMap = new Map(inventories.map(inv => [inv.ingredient, inv]));
