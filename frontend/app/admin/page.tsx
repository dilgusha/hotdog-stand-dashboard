...
//       // Fetch recipes data if not already loaded
//       if (recipe.length > 0)
//         return; // Skip fetching if already loaded

//       console.log('Fetching recipe data...');
//       try {
//         const response = await fetch("http://localhost:5000/api/recipe/recipes");
//         const data = await response.json();
//         console.log(data);
//         setRecipe(data); // Assuming the data follows the Recipe structure
//       } catch (error) {
...
//                     // Use quantityNeeded for display and check if it's greater than 10 for unit selection
//                     const isGrams = recipeItem.quantityNeeded > 10;

//                     const quantityText = isGrams
//                       ? `${recipeItem.quantityNeeded} qram` // Show quantity in grams if greater than 10
//                       : `${recipeItem.quantityNeeded} ədəd`; // Show quantity in units if less than or equal to 10
...
//                             size="sm"
//                             variant="outline"
//                             onClick={() => updateIngredient(recipeItem.id, Math.max(0, recipeItem.ingredient.quantity - 1))}
//                           >
//                             <Minus className="h-3 w-3" />
//                           </Button>
//                           <input
//                             type="number"
//                             value={recipeItem.ingredient.quantity}
//                             onChange={(e) =>
//                               updateIngredient(recipeItem.id, Math.max(0, Number(e.target.value)))
//                             }
...
//                             size="sm"
//                             variant="outline"
//                             onClick={() => updateIngredient(recipeItem.id, recipeItem.ingredient.quantity + 1)}
//                           >
//                             <Plus className="h-3 w-3" />
//                           </Button>
//                         </div>

//                         {/* Displaying quantity with either 'qram' or 'ədəd' */}
//                         <div className="mb-3">
//                           <span className="text-lg font-medium">
//                             {recipeItem.ingredient.quantity}{" "}
//                             {isGrams ? "qram" : "ədəd"}
//                           </span>
//                         </div>
...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Calendar, CalendarDays, TrendingUp, Package, LogOut, Plus, Minus } from "lucide-react";
import { useRouter } from "next/navigation";
import { toast } from "@/hooks/use-toast";
...
export default function AdminPage() {
  ...
  const updateIngredient = async (recipeId: string, newQuantity: number) => {
    const previousRecipes = [...recipe];
    const updatedRecipes = recipe.map((r) =>
      r.id === recipeId ? { ...r, ingredient: { ...r.ingredient, quantity: newQuantity } } : r
    );
    ...
    try {
      const recipeItem = recipe.find((r) => r.id === recipeId);
      if (!recipeItem) throw new Error("Recipe not found");

      const res = await fetch(`http://localhost:5000/api/inventory/${recipeItem.ingredient.id}/quantity`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      toast({
        title: "Запасы обновлены",
        description: `${recipeItem.ingredient.ingredient}: ${newQuantity}`,
      });
    } catch (error: any) {
      setRecipe(previousRecipes);
      toast({
                  ...
                  {recipe.length > 0 ? (
                    recipe.map((recipeItem) => {
                      ...
                      return (
                        <div key={recipeItem.id} className="border rounded-lg p-4">
                          <h3 className="font-medium mb-3">{recipeItem.ingredient.ingredient}</h3>
                          <p className="text-sm text-gray-600 mb-2">Требуется: {quantityText}</p>
                          <div className="flex items-center gap-2 mb-3">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() =>
                                updateIngredient(recipeItem.id, Math.max(0, recipeItem.ingredient.quantity - 1))
                              }
                            >
                              <Minus className="h-3 w-3" />
                            </Button>
                            ...
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                updateIngredient(recipeItem.id, recipeItem.ingredient.quantity + 1)
                              }
                            >
                              <Plus className="h-3 w-3" />
                            </Button>
                          ...
                          <div className="mb-3">
                            <span className="text-lg font-medium">
                              {recipeItem.ingredient.quantity} {isGrams ? "qram" : "ədəd"}
                            </span>
                          </div>
                          <Badge
                            variant={
