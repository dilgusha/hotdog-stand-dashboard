...
//                         ? `${recipeItem.ingredient.quantity} грам`
//                         : `${recipeItem.ingredient.quantity} шт`;
//                       const requiredText = isGrams
//                         ? `${recipeItem.quantityNeeded} грам`
//                         : `${recipeItem.quantityNeeded} шт`;
...
export default function EmployeePage() {
  ...
  //     toast({
  //       title: "Успешно",
  //       description: `Заказ создан на сумму ${result.totalAmount || 0}₼`,
  //     });
  ...
  // Local toplam hesaplama
  const localTotal = cart.reduce((sum, item) => {
    if ("category" in item) {
      const addonTotal = (item.addonIds || []).reduce((addSum, addonId) => {
        const addon = addons.find((a) => a.id === addonId);
        return addSum + (addon?.price || 0) * (item.quantity || 1);
  ...
  try {
    ...
    // Backend'den gelen totalAmount'ı kontrol et, yoksa local toplamı kullan
    const finalTotal = result.totalAmount || localTotal;

    toast({
      title: "Успешно",
      description: `Заказ создан на сумму ${finalTotal.toFixed(2)}₼`,
    });
                    ...
                    Object.values(
                      ...
                      return (
                        <div key={aggregatedIngredient.id} className="border rounded-lg p-4">
                          ...
                          {/* <p className="text-sm text-gray-600 mb-2">Требуется: {quantityNeededText}</p> */}
                          <div className="mb-3">
                            <span className="text-lg font-medium">{quantityText}</span>
                          </div>
                          <Badge
