...
// // Type for the category grouping object
...
//         console.log("Product Data:", productData); // Debug the full response
...
//         })) : [];
//         console.log("Mapped Menu Items by Category:", mappedMenuItems.reduce((acc: CategoryMap, item) => ({
//           ...acc,
//           [item.category]: (acc[item.category] || []).concat(item),
//         }), {})); // Type-safe reduction
...
//       "ADDON": ProductCategory.ADDON,
//       "COMBOS": ProductCategory.COMBOS,
//       "beverage": ProductCategory.DRINK,
...
//         sauceQuantities: item.sauceQuantities || {},
//       })),
//       total,
...
//     toast({ title: "Заказ добавлен", description: `Заказ на сумму ${total}₼ успешно создан` });
//     setCart([]);
//   };
...
//                                 {addons.filter(a => ["Extra Ketchup", "Extra Mustard", "Extra BBQ Sauce"].includes(a.name)).map((addon) => (
//                                   <div key={addon.id} className="flex items-center gap-2 mt-1">
//                                     <span>{addon.name} (+{addon.price}₼):</span>
//                                     <Button size="sm" variant="outline" onClick={() => updateSauceQuantity(item.id, addon.id, (item.sauceQuantities?.[addon.id.toString()] || 0) - 1)}>
//                                       <Minus className="h-3 w-3" />
//                                     </Button>
